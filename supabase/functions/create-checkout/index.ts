...
serve(async (req) => {
  ...
  const rl = await checkRateLimit(clientKey(req, "create-checkout"), { limit: 10, windowMs: 60_000 });
  if (!rl.ok) {
  ...
  try {
    ...
    const origin = req.headers.get("origin") || "https://design-quest-win.lovable.app";
